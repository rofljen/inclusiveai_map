...
export default function SubfamilyPage({ params }: Props) {
  const [languages, setLanguages] = useState<LanguageData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const subfamilyName = decodeURIComponent(params.name);
  ...
  useEffect(() => {
    const fetchLanguages = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch(`/api/languages/subfamily/${encodeURIComponent(subfamilyName)}`);
        if (!response.ok) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">Languages in {subfamilyName} Subfamily</h1>
        ...
        {languages.map((lang) => (
          ...
          >
            ...
            <div className="text-sm text-gray-500">
              ...
              {lang.family_name && <p>Family: {lang.family_name}</p>}
